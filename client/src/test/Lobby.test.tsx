...
it('방 생성 후 /back-room/:roomId 로 이동', async () => {
  server.use(http.post('/api/match/create', () => HttpResponse.json({ roomId: 'r2', status: 'waiting', host: { id: 'u1' } })))
  renderWithProviders(<Lobby />, {
    route: '/lobby',
    routes: [
      <Route key="lobby" path="/lobby" element={<Lobby />} />,
...
it('방 참가 실패 시 에러 토스트 노출', async () => {
  server.use(
    http.get('/api/decks', () => HttpResponse.json([{ id: 'd1', name: '기본 덱', main_cards: [], cata_cards: [], created_at: '', updated_at: '' }])),
    http.get(/https?:\/\/.*\/api\/decks$/, () => HttpResponse.json([{ id: 'd1', name: '기본 덱', main_cards: [], cata_cards: [], created_at: '', updated_at: '' }])),
    http.post('/api/match/join', () => HttpResponse.json({ message: 'join failed' }, { status: 500 })),
    http.post(/https?:\/\/.*\/api\/match\/join$/, () => HttpResponse.json({ message: 'join failed' }, { status: 500 })),
  )
  renderWithProviders(<Lobby />, {
    route: '/lobby',
    routes: [
...
it('덱 카드 섹션: 수정/삭제/새 덱 만들기 동작', async () => {
  ...
  // 삭제 실패 토스트
  server.use(
    http.delete('/api/decks/:deckId', () => HttpResponse.json({ message: 'delete failed' }, { status: 500 })),
    http.delete(/https?:\/\/.*\/api\/decks\/.+$/, () => HttpResponse.json({ message: 'delete failed' }, { status: 500 })),
  )
  fireEvent.click(delBtn)
