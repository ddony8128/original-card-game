...
it('카드 추가 및 제한 일부 동작 확인', async () => {
  ...
  const fire =
    (await screen.findAllByText(/파이어볼/)).find((el) => el.tagName === 'H3') ??
    (await screen.findAllByText(/파이어볼/))[0];
  const blaze =
    (await screen.findAllByText(/블레이즈/)).find((el) => el.tagName === 'H3') ??
    (await screen.findAllByText(/블레이즈/))[0];
  ...
  const doom =
    (await screen.findAllByText(/파멸/)).find((el) => el.tagName === 'H3') ??
    (await screen.findAllByText(/파멸/))[0];
