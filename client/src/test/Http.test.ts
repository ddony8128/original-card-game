...
it('http 에러 시 ApiError(message) 던짐', async () => {
  server.use(
    mswHttp.get('/api/auth/me', () => HttpResponse.json({ message: 'invalid token' }, { status: 401 })),
    mswHttp.get(/https?:\/\/.*\/api\/auth\/me$/, () => HttpResponse.json({ message: 'invalid token' }, { status: 401 })),
  )
