...
describe("logsService", () => {
  it("createGameResult and updateGameResult, getFinishedGameResult", async () => {
    ...
    const room = { id: crypto.randomUUID(), code: "RMTEST1", status: "playing", host_id: "h1", guest_id: "g1", host_deck_id: "d1", guest_deck_id: "d2" };
    // seed decks
    tables.decks.push({ id: "d1", main_cards: [], cata_cards: [], name: "d1", user_id: "h1", deleted: false });
    tables.decks.push({ id: "d2", main_cards: [], cata_cards: [], name: "d2", user_id: "g1", deleted: false });
    ...
    const updated = await logsService.updateGameResult(room.code, "p1", endedAt);
    expect(updated).toMatchObject({ roomId: room.code, result: "p1", status: "finished" });
  ...
  it("createTurnLog and getLogs", async () => {
    const tables = __getTables();
    const room = { id: crypto.randomUUID(), code: "RMTEST2", status: "playing", host_id: "h1", guest_id: "g1" };
    tables.rooms.push(room as any);
    // minimal game_result
