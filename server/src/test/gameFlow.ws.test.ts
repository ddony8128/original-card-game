...
import type { SocketManager } from '../ws/socketManager';
...
class FakeSocketManager implements Pick<
  SocketManager,
  'joinRoom' | 'leave' | 'broadcast' | 'sendTo'
> {
  public joined: Array<{ room: string; userId?: string }> = [];
  public leftCount = 0;
  public broadcasts: Array<{ room: string; msg: ServerToClientMessage }> = [];
  public directed: Array<{ room: string; userId: string; msg: ServerToClientMessage }> = [];
  ...
  broadcast(roomCode: string, data: unknown): void {
    this.broadcasts.push({ room: roomCode, msg: data as ServerToClientMessage });
  }

  sendTo(roomCode: string, userId: string, data: unknown): void {
    this.directed.push({ room: roomCode, userId, msg: data as ServerToClientMessage });
  }
}
...
describe('WebSocket + GameEngine 통합 플로우 (준단위)', () => {
  it('ready → answer_mulligan → player_action 흐름에서 서버 이벤트가 발생한다 (기본 형태만 검증)', async () => {
    ...
    const roomManager = new GameRoomManager(fakeManager as unknown as SocketManager);
    const roomId = 'ROOM1';
    const p1: PlayerID = 'host-1';
    const p2: PlayerID = 'guest-1';

    // ready 두 번 호출 (엔진 초기화 + 멀리건 요청/상태 패치 발생)
    await roomManager.handleReady(roomId, { roomId, userId: p1 });
    await roomManager.handleReady(roomId, { roomId, userId: p2 });
    ...
    await roomManager.handleAnswerMulligan(roomId, p1, mulliganPayload);
    await roomManager.handleAnswerMulligan(roomId, p2, mulliganPayload);

    // 이후 한 플레이어가 턴 종료 액션을 보낸다고 가정
    const endTurn: PlayerActionPayload = { action: 'end_turn' };
    await roomManager.handlePlayerAction(roomId, p1, endTurn);
