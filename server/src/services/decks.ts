...
export const decksService = {
  async validateAndHydrate(
    ...
    // count 범위 체크 및 합계 검증
    const isValidCount = (n: number) =>
      typeof n === 'number' && Number.isFinite(n) && (n === 1 || n === 2);
    if (!Array.isArray(main) || !Array.isArray(cata))
      throw new Error('invalid deck payload');
    if (!main.every((e) => typeof e.id === 'string' && isValidCount(e.count)))
      throw new Error('invalid main_cards entries');
    if (!cata.every((e) => typeof e.id === 'string' && isValidCount(e.count)))
      throw new Error('invalid cata_cards entries');

    const mainCountSum = main.reduce((s, e) => s + e.count, 0);
    const cataCountSum = cata.reduce((s, e) => s + e.count, 0);
    if (mainCountSum !== 16)
      throw new Error('main_cards must contain exactly 16 cards');
    if (cataCountSum !== 4)
      throw new Error('cata_cards must contain exactly 4 cards');
    ...
    const hydratedMain: DeckList = main.map((e) => {
      ...
      if (row.type === 'catastrophe')
        throw new Error(`catastrophe not allowed in main: ${e.id}`);
      return {
        id: e.id,
        count: e.count,
    ...
    const hydratedCata: DeckList = cata.map((e) => {
      const row = ensureRow(e.id);
      if (row.type !== 'catastrophe')
        throw new Error(`only catastrophe allowed in cata: ${e.id}`);
      return {
        id: e.id,
        count: e.count,
        name_dev: row.name_dev,
  ...
  async listByUser(userId: string): Promise<DeckRow[]> {
    const { data, error } = await supabase
      .from('decks')
      .select(
        'id,user_id,name,main_cards,cata_cards,deleted,created_at,updated_at',
      )
      .eq('user_id', userId)
      .eq('deleted', false)
    ...
    const rows = (data ?? []) as Array<
      Omit<DeckRow, 'main_cards' | 'cata_cards'> & {
        main_cards: unknown;
        cata_cards: unknown;
      }
    >;
  ...
  async create(
    ...
    const toInsert = {
      user_id: userId,
      name,
      main_cards: payload.main_cards,
      cata_cards: payload.cata_cards,
    };
    const { data, error } = await supabase
      .from('decks')
      .insert(toInsert)
  ...
  async getById(deckId: string): Promise<DeckRow | null> {
    ...
    if (error) throw error;
    if (!data) return null;
    const row = data as Omit<DeckRow, 'main_cards' | 'cata_cards'> & {
      main_cards: unknown;
      cata_cards: unknown;
    };
    return {
