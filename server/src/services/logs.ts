...
export const logsService = {
    ...
    async createGameResult(roomCode: string, startedAt: string): Promise<{ resultId: string; roomId: string }> {
    ...
    async updateGameResult(roomCode: string, result: "p1" | "p2" | "draw", endedAt: string) {
        const room = await this.findRoomByCode(roomCode);
        if (!room) throw Object.assign(new Error("room not found"), { code: "NOT_FOUND" });
        ...
        if (!gr) throw Object.assign(new Error("game result not found"), { code: "NOT_FOUND" });

        let winner_id: string | null = null;
        if (result === "p1") winner_id = (gr as any).player1_id ?? room.host_id;
        else if (result === "p2") winner_id = (gr as any).player2_id ?? room.guest_id;
    ...
    async getFinishedGameResult(roomCode: string) {
        const room = await this.findRoomByCode(roomCode);
        if (!room) throw Object.assign(new Error("room not found"), { code: "NOT_FOUND" });
        if (room.status !== "finished") throw Object.assign(new Error("game not finished"), { code: "FORBIDDEN" });
    ...
    async getLogs(roomCode: string): Promise<{ roomExists: boolean; logs: Array<{ turn: number; text: string }> }> {
        const room = await this.findRoomByCode(roomCode);
