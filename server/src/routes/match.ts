...
async function buildStateResponse(roomCode: string) {
    ...
    const [host, guest] = await Promise.all([
        ...
        room.guest_id ? usersService.findById(room.guest_id) : Promise.resolve(null),
    ]);
    return {
        roomId: room.code,
        host: host
...
matchRouter.patch("/deck", (req, res) => {
    ...
    (async () => {
        ...
        if (!deck)
            return res.status(HttpStatus.NOT_FOUND).json({ message: "deck not found" });
        if (deck.deleted)
            return res.status(HttpStatus.BAD_REQUEST).json({ message: "deck deleted" });
        if (deck.user_id !== userId)
            return res.status(HttpStatus.FORBIDDEN).json({ message: "forbidden: not owner" });
