...
logsRouter.post("/result", requireInternal, (req, res) => {
  const { roomId, startedAt } = req.body as { roomId?: string; startedAt?: string };
  if (!roomId || !startedAt) {
    return res.status(HttpStatus.BAD_REQUEST).json({ message: "roomId and startedAt required" });
  }
  (async () => {
    ...
    return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: (e as any).message });
  });
});
...
logsRouter.patch("/result/:roomId", requireInternal, (req, res) => {
  ...
  const { result, endedAt } = req.body as { result?: "p1" | "p2" | "draw"; endedAt?: string };
  if (!result || !endedAt) {
    return res.status(HttpStatus.BAD_REQUEST).json({ message: "result and endedAt required" });
  }
  (async () => {
...
logsRouter.get("/result/:roomId", requireAuthOrInternal, (req, res) => {
  ...
  (async () => {
    ...
    const code = (e as any).code;
    if (code === "FORBIDDEN") {
      return res.status(HttpStatus.FORBIDDEN).json({ message: "game not finished" });
    }
    if (code === "NOT_FOUND") {
      return res.status(HttpStatus.NOT_FOUND).json({ message: e.message });
    }
...
logsRouter.post("/log", requireInternal, (req, res) => {
  const { resultId, turn, text } = req.body as { resultId?: string; turn?: number; text?: string };
  if (!resultId || typeof turn !== "number" || typeof text !== "string") {
    return res.status(HttpStatus.BAD_REQUEST).json({ message: "resultId, turn, text required" });
  }
  (async () => {
    await logsService.createTurnLog(resultId, turn, text);
    return res.status(HttpStatus.CREATED).json({ ok: true });
  })().catch((e) =>
    res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ message: (e as any).message })
  );
});
